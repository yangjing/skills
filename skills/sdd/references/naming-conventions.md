---
status: active
version: v4  # 2026-09-18 增 §13 长期产物命名与临时批次编号
---
...
临时执行单元（plan / 迭代 / 审查批次）的编号是**追溯锚，不是命名依据**。产物按生命周期分两类，编号准入不同：

| 产物 | 批次编号准入 | 命名依据 |
| --- | --- | --- |
| 执行计划本体及其过程性产物（evidence、验收记录、归档索引） | MAY——编号的属主 | 计划标题 / 领域 |
| 测试代码（文件名、套件与用例标题、测试内标识符 / 环境变量 / fixture 前缀） | MUST NOT | 领域聚合根或领域（如 `trash-oplog`、`attachment-cache`） |
| 规格 / 设计 / 规范文档 | MUST NOT | 领域聚合根或领域 |
| 通用代码标识符（函数 / 变量 / 类型 / 示例二进制名） | MUST NOT | 所在语言规范（§3 / §11） |

- 判据：plan 是**临时性**的——编号只为追溯；代码与规格是**长期性**的——会被多个批次修改，且可能拆分或合并。批次编号命名的产物在拆并后语义即失效，且不表达「属于哪个领域」。
- 溯源保留面：注释、commit message、执行计划正文中的批次编号引用属追溯信息，MUST 保留，不在本节射程。
- 同名消歧：测试标识符剥去批次编号后与被测符号同名时，MUST 补领域场景后缀（如 `_edges` / `_bounds`）消歧，MUST NOT 恢复编号前缀。
- 存量治理：发现批次编号命名时按领域聚合根重命名，并同批同步全部路径引用（含活文档中的证据指针）；已归档的计划文档属历史记录，MUST NOT 回改。
